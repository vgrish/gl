gl.grid.Country = function(config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        stateful: true,
        stateId: 'gl-grid-country-state'

    });
    gl.grid.Country.superclass.constructor.call(this, config);

};
Ext.extend(gl.grid.Country, MODx.grid.Grid, {
    windows: {},

    getFields: function(config) {
        var fields = ['id', 'iso', 'continent', 'name', 'name_ru', 'lat', 'lon', 'timezone', 'active', 'actions'];
            ...
            listeners: {
                success: {
                    fn: function(r) {
                        var record = r.object;
                        var w = MODx.load({
                            xtype: 'gl-window-create-country',
...
Ext.reg('gl-grid-country', gl.grid.Country);
