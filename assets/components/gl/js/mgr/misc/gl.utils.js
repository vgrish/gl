...
gl.utils.getMenu = function(actions, grid, selected) {
    var menu = [];
...
gl.utils.renderActions = function(value, props, row) {
    var res = [];
    ...
    return String.format(
        '<ul class="gl-row-actions">{0}</ul>',
        res.join('')
    );
};

gl.utils.handleChecked = function(checkbox) {
...
gl.utils.renderReplace = function(value, replace, color) {
    if (!value) {
        return '';
    } else if (!replace) {
        return value;
    ...
    return String.format('<span class="gl-render-color" style="color: #{1}">{0}</span>', replace, color);
};
